...
// list = รายการเรียบง่ายของวันนี้ (แยกจาก "สรุป"/"วันนี้" ที่เป็นการ์ดกราฟ)
const CMD_LIST = /^(รายการ|ลิสต์|list|รายการวันนี้)$/i;
// undo: ไม่จับ "ลบ" เดี่ยว — นั่นเป็นของโมดูล todo
const CMD_UNDO = /^(ยกเลิก|ลบล่าสุด|undo)$/i;

/** คำนำหน้าสำหรับ "จดรายการ" — ต้องมี "จด" ตามด้วยเว้นวรรค/ขึ้นบรรทัดใหม่/จบ (บรรทัดแรกเท่านั้น).
 *  boundary \s|$ กัน "จดหมาย 50" (จด+หมาย ติดกัน) ไม่ให้กลายเป็นรายการ. group1 = รายการแรก
 *  ที่พิมพ์ต่อบนบรรทัดเดียวกับ "จด" (ถ้ามี) — รายการที่เหลือมาจากบรรทัดถัด ๆ ไป. */
const RECORD_PREFIX = /^จด(?:\s+(.+))?$/;

/**
 * แปลงข้อความ → LedgerIntent. ตรวจคำสั่ง "ดู" ทั้งข้อความก่อน (day/week/month/report/list/undo)
 * — คำสั่งเหล่านี้ไม่ต้องมี "จด". ส่วน "การเพิ่มรายการ" ต้องขึ้นต้นด้วย **"จด"** เท่านั้น แล้ว
 * พิมพ์รายการต่อทีละบรรทัด (หรือบรรทัดเดียวกับ "จด" ก็ได้) — กันข้อความทั่วไปในกลุ่ม (เช่น "555")
 * ไม่ให้ถูกจดโดยไม่ตั้งใจ. ข้อความที่ไม่ใช่คำสั่งและไม่ขึ้นต้น "จด" → null (บอทเงียบ).
 */
export function parseLedgerIntent(text: string, now: Date): LedgerIntent {
  const trimmed = (text ?? "").trim();
  if (!trimmed) return null;

  // คำสั่ง "ดู/จัดการ" (ทั้งข้อความตรงกับ pattern เดียว) — ไม่ต้องมี "จด"
  ...
  if (CMD_LIST.test(trimmed)) return { action: "list" };
  if (CMD_UNDO.test(trimmed)) return { action: "undo" };

  // เพิ่มรายการ — ต้องขึ้นต้นด้วย "จด" บนบรรทัดแรก
  const lines = trimmed.split("\n");
  const prefixMatch = lines[0].trim().match(RECORD_PREFIX);
  if (!prefixMatch) return null; // ไม่ใช่คำสั่ง และไม่ขึ้นต้น "จด" → ไม่จด

  const today = bkkToday(now);
  const firstRemainder = (prefixMatch[1] ?? "").trim();
  const recordText = [firstRemainder, ...lines.slice(1)].filter(Boolean).join("\n");
  const entries = parseEntries(recordText, today);
  // แม้ยังไม่มีรายการที่ถูกต้อง ก็คืน record (entries ว่าง) เพื่อให้ handler แนะนำวิธีพิมพ์
